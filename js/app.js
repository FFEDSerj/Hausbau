...
let slider_review = new Swiper('.content-screen__slider-review', {
  /*
  effect: 'fade',
  autoplay: {
    delay: 3000,
  ...
  */
  ...
  // Arrows
  navigation: {
    nextEl: '.content-screen__arrow-next',
    prevEl: '.content-screen__arrow-prev',
  },
  /*
  breakpoints: {
    320: {
...
let pageSlider = new Swiper('.page', {
  ...
  // Стрелки навигации
  navigation: {
    nextEl: '.page__button-next',
    prevEl: '.page__button-prev',
  },
  ...
  on: {
    ...
    //   document.addEventListener("scroll", function () { document.getElementById("simple-operation").scrollIntoView({ behavior: "smooth", }); });
    // },
    resize: function () {
      setScrollType();
    }
...
// bg Change on MouseOver
let screens = document.querySelectorAll(".screen");

for (let index = 0; index < screens.length; index++) {
  let screen = screens[index];

  let contentItems = screen.querySelectorAll(".item-content-block");
  let bgChanger = screen.querySelectorAll(".screen__section-bg");
  for (let index = 0; index < contentItems.length; index++) {
    let contentItem = contentItems[index];
    contentItem.addEventListener("mouseover", function (e) {
      for (let index = 0; index < contentItems.length; index++) {
        let contentItem = contentItems[index];
        contentItem.classList.remove('_active');
        bgChanger[index].classList.remove('_active');
      }
      contentItem.classList.add('_active');
      bgChanger[index].classList.add('_active');
      e.preventDefault();
    });
  }
}
